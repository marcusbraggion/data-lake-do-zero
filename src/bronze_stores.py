...
def load_csvs_from_s3_to_dataframe(bucket_name, s3_folder):
    ...
    if dataframes:
        ...
        combined_df = (
            pd.DataFrame()
        )  # Retorna um DataFrame vazio se nenhum CSV for encontrado
...
s3.meta.client.put_object(
    Body=df_latest_year.to_csv(index=False),
    Bucket=bucket_bronze_name,
    Key=f"stores-bronze/stores_bronze.csv",
)
